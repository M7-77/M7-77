<h1 align="center">Hi 👋, I'm Mohamed Karim Dahmouni</h1>
<h3 align="center">Software Engineering Student at INSAT, Tunisia | Backend & Discord Bot Developer</h3>

---

### 🚀 About Me

- 🎓 2nd-year **Software Engineering** student at **INSAT**, Tunisia
- 💻 Writing code since **2020**
- 🤖 Built and maintained **2 Discord-verified bots** (2020–2023), backed by `Node.js`, `MongoDB`, `discord.js`, and external REST APIs
- 🎛️ Designed a custom **React dashboard** to manage and monitor those bots
- 🐍 Built several **Python tools** for Discord automation
- 🌱 Currently leveling up: Data Structures & Algorithms, TypeScript, system design
- 🇹🇳 Based in Tunisia

---

### 🛠️ Tech Stack

<p align="left">
  <img src="https://skillicons.dev/icons?i=c,js,html,css,py,nodejs,react,mongodb,git,github" />
</p>

<p align="left">
  <img src="https://img.shields.io/badge/discord.js-5865F2?style=for-the-badge&logo=discord&logoColor=white" />
  <img src="https://img.shields.io/badge/REST_APIs-009688?style=for-the-badge&logo=fastapi&logoColor=white" />
</p>

---

### 📊 GitHub Stats

<p align="left">
  <img height="165" src="https://github-readme-stats.vercel.app/api?username=YOUR_GITHUB_USERNAME&show_icons=true&theme=tokyonight&hide_border=true" />
  <img height="165" src="https://github-readme-streak-stats.herokuapp.com/?user=YOUR_GITHUB_USERNAME&theme=tokyonight&hide_border=true" />
</p>

---

### 📫 Connect With Me

<p align="left">
  <a href="mailto:mohamedkarim.dahmouni@insat.ucar.tn"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" /></a>
  <a href="https://www.linkedin.com/in/mohamed-karim-dahmouni-438773255/"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" /></a>
</p>
